...
## 9. Ready-to-Run Test Assets

This repository also includes reusable non-website testing assets:

- `tools/testing/postman/AutoAttendance.postman_collection.json` for Postman imports.
- `tools/testing/scripts/test-api.ps1` for Windows PowerShell health checks and video upload tests.
- `tools/testing/scripts/test-api.sh` for Git Bash, macOS, and Linux health checks and video upload tests.

PowerShell example:

```powershell
.\tools\testing\scripts\test-api.ps1 -VideoPath C:\path\to\classroom-video.mp4
```

Bash example:

```bash
BASE_URL=http://localhost:8000 ./tools/testing/scripts/test-api.sh /path/to/classroom-video.mp4
```
